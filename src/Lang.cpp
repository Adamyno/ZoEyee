...
const char *const lang_PAGES_DESC[] = {"Number of dashboard pages",
                                       "Muszerfal oldalak szama"};
const char *const lang_TAP_ARROWS[] = {"Tap arrows to adjust",
                                       "Nyilakkal allitsd"};
const char *const lang_TAP_TOGGLE[] = {"(tap to toggle)",
                                       "(erintsd a valtashoz)"};
...
const char *const lang_FMT_FLASH[] = {"Free Flash: %u / %uKB",
                                      "Flash: %u / %u KB"};
const char *const lang_FMT_RAM[] = {"Free RAM: %u / %uKB", "RAM: %u / %u KB"};
const char *const lang_FMT_UPTIME[] = {"Uptime: %02d:%02d:%02d",
                                       "Uzemido: %02d:%02d:%02d"};
...
const char *const lang_SWIPE_BACK[] = {"Swipe Right -> Back",
                                       "Jobbra huzas -> Vissza"};
...
const char *const lang_NO_OBD[] = {"No OBD service found!",
                                   "Nincs OBD szolgaltatas!"};
...
const char *const lang_STATUS_CONNECTED[] = {"Status: Connected",
                                             "Allapot: Csatlakozva"};
const char *const lang_STATUS_RECONNECTING[] = {"Status: Reconnecting...",
                                                "Allapot: Ujracsatl..."};
...
const char *const lang_SAVE_AUTO[] = {"Save & Auto-Conn",
                                      "Mentes & Auto-Csatl."};
...
const char *const lang_paramNames[][LANG_COUNT] = {
    ...
    {"Cabin Temp", "Belso homerseklet"},         // 2
    {"Battery Temp", "Akku homerseklet"},        // 3
    {"AC Compressor", "Klima kompr. ford.szam"}, // 4
    {"AC Pressure", "Klima nyomas"},             // 5
    {"12V Battery", "12V Akku"},                 // 6
    {"Ext. Temp", "Kulso homerseklet"},          // 7
    {"Cell Delta V", "Cella Delta V"},           // 8
    {"Cell V Max", "Cella Max Feszultseg"},      // 9
    {"Cell V Min", "Cella Min Feszultseg"},      // 10
    {"Engine Fan", "Motor ventilator"},          // 11
    {"Climate Mode", "Klima mod"},               // 12
    {"Max Charge", "Max. toltesi sebesseg"},     // 13
    {"DC Power", "DC teljesitmeny"},             // 14
    {"Avail Energy", "Elerheto toltottseg"},     // 15
    {"HV Voltage", "HV feszultseg"},             // 16
    {"AC Phase", "AC fazis"},                    // 17
    {"Insulation", "Szigetelesi ellenallas"},    // 18
};
